...
class UnPayConfig
{
    ...
    // 签名证书路径
    const SDK_SIGN_CERT_PATH = '﻿﻿﻿﻿﻿﻿/phpstudy/www/upacp_sdk_php/cert/PM_700000000000001_acp.pfx';
    
    // 签名证书密码
    const SDK_SIGN_CERT_PWD = '000000';
    
    // 验签证书
    const SDK_VERIFY_CERT_PATH = '/phpstudy/www/upacp_sdk_php/cert/verify_sign_acp.cer';
    
    // 密码加密证书（这条用不到的请随便配）
    const SDK_ENCRYPT_CERT_PATH = '/phpstudy/www/upacp_sdk_php/cert/encrypt.cer';
    
    // 验签证书路径（请配到文件夹，不要配到具体文件）
    const SDK_VERIFY_CERT_DIR = '/phpstudy/www/upacp_sdk_php/cert/';
    ...
    // 前台通知地址 (商户自行配置通知地址)
    const SDK_FRONT_NOTIFY_URL = 'http://localhost/upacp_sdk_php/demo/utf8/FrontReceive.php';
    // 后台通知地址 (商户自行配置通知地址)
    const SDK_BACK_NOTIFY_URL = 'http://localhost/upacp_sdk_php/demo/utf8/BackReceive.php';
