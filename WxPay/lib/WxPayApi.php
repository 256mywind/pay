...
class WxPayApi
{
	...
	public static function unifiedOrder($inputObj, $timeOut = 6)
	{
		...
		if(!$inputObj->IsNotify_urlSet()){
			$inputObj->SetNotify_url(JsPayConfig::NOTIFY_URL);//异步通知url
		}
		if (!$inputObj->IsAppidSet()){
		    $inputObj->SetAppid(JsPayConfig::APPID);//公众账号ID
		}
		if (!$inputObj->IsMch_idSet()){
		    $inputObj->SetMch_id(JsPayConfig::MCHID);//商户号
		}
	...
	public static function orderQuery($inputObj, $timeOut = 6)
	{
		...
		if (!$inputObj->IsAppidSet()) {
		    throw new WxPayException('请设置APPID');
		}
		if (!$inputObj->IsMch_idSet()) {
		    throw new WxPayException('请设置MCHID');
		}
		$inputObj->SetNonce_str(self::getNonceStr());//随机字符串
	...
	{
		//如果不需要上报数据
		if(JsPayConfig::REPORT_LEVENL == 0){
			return;
		} 
		//如果仅失败上报
		if(JsPayConfig::REPORT_LEVENL == 1 &&
	...
	{		
		...
		if($useCert == true){
			...
			curl_setopt($ch,CURLOPT_SSLCERT, JsPayConfig::SSLCERT_PATH);
			curl_setopt($ch,CURLOPT_SSLKEYTYPE,'PEM');
			curl_setopt($ch,CURLOPT_SSLKEY, JsPayConfig::SSLKEY_PATH);
		}
		//post提交方式
