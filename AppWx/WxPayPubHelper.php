...
class Wxpay_client_pub extends Common_util_pub 
{
	...
	function createXml()
	{
	   	$this->parameters["appid"] = C('wxpay.APPID');//公众账号ID
	   	$this->parameters["mch_id"] = C('wxpay.MCHID');//商户号
...
class NativeCall_pub extends Wxpay_server_pub
{
	...
	function createXml()
	{
		if($this->returnParameters["return_code"] == "SUCCESS"){
		   	$this->returnParameters["appid"] = C('wxpay.APPID');//公众账号ID
		   	$this->returnParameters["mch_id"] = C('wxpay.MCHID');//商户号
...
class JsApi_pub extends Common_util_pub
{
	...
	{
		$urlObj["appid"] = C('wxpay.APPID');
		$urlObj["secret"] = C('wxpay.APPSECRET');
	...
	//判断$access_token是否有效
    function checkaccess_token($openid,$access_token){
    ...
    function refreshToken($refresh_token){
        $url = "https://api.weixin.qq.com/sns/oauth2/refresh_token?appid=".C('wxpay.APPID')."&grant_type=refresh_token&refresh_token=$refresh_token";
        //初始化curl
        $ch = curl_init();
        //设置超时
        curl_setopt($ch, CURLOP_TIMEOUT, 30);
